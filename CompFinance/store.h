...
extern ProductStore productStore;

inline void putBlackScholes(
    const double            spot,
    const double            vol,
    const bool              qSpot,
    const double            rate,
...
template<class T>
inline const Model<T>* getModel(const string& store);

template<>
inline const Model<double>* getModel(const string& store)
{
...
template<>
inline const Model<Number>* getModel(const string& store)
{
    auto it = modelStore.find(store);
    if (it == modelStore.end()) return nullptr;
...
inline pair<const vector<string>*, const vector<double*>*> getModelParameters(const string& store)
{
    auto it = modelStore.find(store);
...
template<class T>
inline const Product<T>* getProduct(const string& store);

template<>
inline const Product<double>* getProduct(const string& store)
{
...
template<>
inline const Product<Number>* getProduct(const string& store)
{
    auto it = productStore.find(store);
    if (it == productStore.end()) return nullptr;
    else return it->second.second.get();
...
inline const vector<string>* getPayoffLabels(const string& store)
{
    auto it = productStore.find(store);
    if (it == productStore.end()) return nullptr;
