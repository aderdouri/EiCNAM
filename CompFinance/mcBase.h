...
struct SampleDef
{
    ...
    struct RateDef
    {
        ...
        RateDef(const Time s, const Time e, const string &c) : start(s), end(e), curve(c) {};
    };
...
{
    AADSimulResults(const size_t nPath, const size_t nPay, const size_t nParam) : payoffs(nPath, vector<double>(nPay)),
                                                                                  aggregated(nPath),
                                                                                  risks(nParam)
    {
    }
...
const auto defaultAggregator = [](const vector<Number> &v)
{ return v[0]; };
    ...
    const F &aggFun = defaultAggregator)
{
    ...
    std::cout << "After AADSimulResults..." << std::endl;
    ...
    transform(
        ...
        params.end(),
        results.risks.begin(),
        [nPath](const Number *p)
        { return p->adjoint() / nPath; });
    ...
    const F &aggFun = defaultAggregator)
{
    ...
    vector<vector<double>> gaussVecs(nThread + 1, vector<double>(models[0]->simDim()));
...
{
    AADMultiSimulResults(const size_t nPath, const size_t nPay, const size_t nParam) : payoffs(nPath, vector<double>(nPay)),
                                                                                       risks(nParam, nPay)
    {
    }
