#if !defined(MAIN_H_ALREADY_INCLUDED)
#define MAIN_H_ALREADY_INCLUDED
/*
    ...
    const NumericalParam &num)
{
    ...
    results.delta = (inner_product(vnots.begin(), vnots.end(), bumpedVals.values.begin(), 0.0) - results.value) * 1.0e+08;
    ...
    for (size_t i = 0; i < n; ++i)
        for (size_t j = 0; j < m; ++j)
        {
            ...
            auto bumpedVals = value(bumpedModel, *product, num);
            //  Pick results and differentiate
            results.vega[i][j] = (inner_product(vnots.begin(), vnots.end(), bumpedVals.values.begin(), 0.0) - results.value) * 1.0e+05;
            //  Unbump
            riskView.bump(i, j, -1.0e-05);
        }
