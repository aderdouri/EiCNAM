...
struct Tape
{
    ...
    {
        if (op == OpType::LOG || op == OpType::SIN || op == OpType::COS || op == OpType::EXP)
        {
            rhs = nullptr; // Unary operations do not use rhs
        }
    ...
    void propagateAdjoints()
    {
        ...
        for (int i = nodes.size() - 1; i >= 0; --i)
        {
            ...
            {
                if (node->lhs)
                    node->lhs->adjoint += node->adjoint * (node->rhs ? node->rhs->value : 1.0);
                if (node->rhs)
                    node->rhs->adjoint += node->adjoint * (node->lhs ? node->lhs->value : 1.0);
            }
...
T f(T x[5])
{
    T y1 = x[2] * ((5.0 * x[0]) + x[1]); // FIXED multiplication issue
    T y2 = y1.log();
    T y = (y1 + x[3] * y2) * (y1 + y2);
    return y;
}
